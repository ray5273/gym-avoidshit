...
class AvoidShitEnv(gym.Env):
	...
	def _get_game_state(self):
		...
		state = np.concatenate([self.last_ddong_x, self.last_ddong_y])
		state = np.append(state, self.man_x)
		state = np.append(state, self.man_y)
		state = [float(i)/sum(state) for i in state]
		return state
	...
	def step(self, action):
		...
		self.man_x += dx
		self.man_x = np.clip(self.man_x, 0, self.PAD_WIDTH-self.man_width)
		#if self.man_x <0:
		#	 self.man_x = 0
		...
		for index,value in enumerate(self.last_ddong_y):
			...
			if value > self.PAD_HEIGHT:
				if self.RANDOM:
					self.last_ddong_x[index] = int(random.randrange(0,self.PAD_WIDTH - self.man_width)/48.0)*48.0
				self.last_ddong_y[index] = -self.ddong_height
				self.reward = 1
				self.score += 1
		for index,value in enumerate(self.last_ddong_y):
			if abs(self.last_ddong_x[index] - self.man_x) < self.ddong_width and \
					abs(self.man_y - self.last_ddong_y[index]) < self.ddong_height:
				self.done = True
				self.reward = -1
	...
	def _fill_board(self):
		...
		for i in range(self.total_ddong):
			ddong_x.append(fixed_ddong_x[i]*48.0)
			ddong_y.append(fixed_ddong_y[i]*-32.0)
